...
async function initDb(): Promise<void> {
  try {
    ...
    // Check if the table is empty, and if so, add a test record
    const countResult = await pool.query('SELECT COUNT(*) FROM risk_data');
    if (parseInt(countResult.rows[0].count) === 0) {
      console.log("Adding test record to risk_data table");
      await pool.query(`
      ...
      `, ['Test User', 'Test Hotspot', 'Location A', 1, 7.5, 3.0, 2.5, 'Medium', 'Test Metric', 'Q2 2025']);
...
// Simpler, more direct file upload endpoint
        ...
        if (isRiskData) {
          ...
          for (const row of fileRows) {
            try {
              ...
              // Prepare and log parameters for debugging
              const params = [
                row['Respondent Type'] || '',
                row['Hotspot'] || '',
              ...
              console.log(`Row inserted with ID: ${result.rows[0]?.id || 'unknown'}`);
            } catch (insertError) {
...
// We'll implement error handling in the main Express app instead
// This will make it more reliable for catching Multer errors

// Get risk data endpoint - with limit parameter support
router.get('/risk-data', asyncHandler(async (req: Request, res: Response) => {
  const limit = req.query.limit ? parseInt(req.query.limit as string) : 1000;
  const result = await pool.query("SELECT * FROM risk_data ORDER BY created_at DESC LIMIT $1", [limit]);
  console.log(`Returning ${result.rowCount} risk data rows`);
